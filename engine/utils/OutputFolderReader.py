...
class OutputFolderReader:
    ...
    def _get_report_dirs(self):
        ...
        report_dirs = filter(lambda dir: os.path.isdir(dir), [os.path.join(self.output_folder, report_dir_template.replace(r'{{report_id}}', report_id)) for report_id in self._get_report_ids()])
    ...
    def _read_file_from_each_report_dir(self, file_name_templates: list[str] | str, processing_function, filter_report = lambda report_id: True):
        ...
        for report_dir, report_id in tqdm(zip(self._get_report_dirs(), self._get_report_ids())):
            ...
            file_name_templates = file_name_templates if isinstance(file_name_templates, list) else [file_name_templates]
    ...
    def process_reports_with_specific_files(self, processing_function, file_names, report_filter = None):
        """"
        ...
        """
        file_name_templates = [self.output_config.get("reports").get(file_name) for file_name in file_names]

        # Check to see if any are None
        if any(file_name_template is None for file_name_template in file_name_templates):
            print("  File name templates could not all be found.")
            return
        
        if report_filter is not None:
            self._read_file_from_each_report_dir(file_name_templates, processing_function, report_filter)
        else:
            self._read_file_from_each_report_dir(file_name_templates, processing_function)
    ...
    def read_all_themes(self, processing_function, modes = Modes.all_modes):
        self.process_reports_with_specific_files(processing_function, ["themes_file_name"], lambda report_id: Modes.get_report_mode_from_id(report_id) in modes)
