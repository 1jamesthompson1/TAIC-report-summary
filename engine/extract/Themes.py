    ...
    def __init__(self, output_path: str = None, use_predefined: bool = False):
        ...
        if output_path is None:
            self._file_path = os.path.join(
                engine_config.get("folder_name"),
                engine_config.get("predefined_themes_file_name")
                if use_predefined
                else engine_config.get("themes_file_name"),
            )
        else:
            self._file_path = os.path.join(
                output_path,
...
class ThemeReader(ThemeFile):
    ...
    def get_theme_titles(self, modes=None) -> list:
        if modes is None:
            modes = self._modes
        return [
            theme["title"] for theme in ThemeReader._filter_themes(self._themes, modes)
        ]
    ...
    def _filter_themes(themes, modes):
        ...
        return list(
            filter(
                lambda theme: any(Modes.Mode[mode] in modes for mode in theme["modes"]),
                themes,
            )
        )
