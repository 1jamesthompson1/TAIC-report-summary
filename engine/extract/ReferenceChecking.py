...
class Reference:
    ...
    def __init__(self, text: str, reference_str: str, type: ReferenceType):
        ...
        self.reference = self._parse_reference(
            reference_str
        )  # This is the reference pointing to a partiuclar section, paragraph, or subparagraph.
        self.type = type
    ...
    def _parse_reference(self, reference_str: str):
        ...
        if reference_is_range:
            ...
            if start_section == end_section and start_paragraph == end_paragraph:
                reference = [
                    f"{start_section}.{start_paragraph}.{start_subparagraph + i}"
                    for i in range(end_subparagraph - start_subparagraph + 1)
                ]
            elif start_section == end_section:
                reference = [
                    f"{start_section}.{start_paragraph + i}"
                    for i in range(end_paragraph - start_paragraph + 1)
                ]
...
class ReferenceValidator:
    ...
    def validate_references(self, text) -> [Reference]:
        ...
        for reference in references:
            ...
            quote_regex = rf""""{processed_reference.text}" {{0,2}}\({processed_reference.reference_str}\)"""

            citation_regex = rf"""{processed_reference} {{0,2}}\({processed_reference.reference_str}\)"""

            if processed_reference.unrepairable or processed_reference.invalid:
                print(
                    f"   Invalid {reference.type}: {reference.reference_str} for text {reference.text}"
                )
                if processed_reference.type == ReferenceType.citation:
                    text = re.sub(
                        citation_regex,
                        processed_reference.to_string(),
                        text,
                        flags=re.IGNORECASE,
                    )
            ...
            if (
                processed_reference.updated
                and processed_reference.type == ReferenceType.quote
            ):
                self._print(
                    f"  Fixed reference: {processed_reference.old_reference.reference_str} to {processed_reference.reference_str} for text {processed_reference.text}"
                )
                quote_regex = rf""""{processed_reference.text}" {{0,2}}\({processed_reference.old_reference.reference_str}\)"""
                text = re.sub(
                    quote_regex,
                    processed_reference.to_string(),
                    text,
    ...
    def _extract_references(self, text) -> [Reference]:
        ...
        for match in re.finditer(self.reference_regex, text.lower()):
            ...
            if match.group(2) and match.group(3):
                ...
                new_referenece = Reference(
                    match.group(5), match.group(6), ReferenceType.citation
                )
            else:
    ...
    def _validate_reference(self, reference: Reference, attempt_repair: bool):
        ...
        if reference.invalid:
            print(
                f"   Invalid reference (note this wa a bug fix and the program shouldnt ever make it here.) {reference.to_string()}"
            )
            return reference
        source_sections = list(
            map(
                lambda reference: sectionExtractor.extract_section(reference),
                reference.reference,
            )
        )
        ...
        source_sections = list(
            filter(lambda section: section is not None, source_sections)
        )

        source_sections = "\n".join(
            map(lambda str: str.replace("\n", "").lower(), source_sections)
        ).lower()

        match reference.type:
            case ReferenceType.citation:
                return self._validate_citation(
                    reference, source_sections, attempt_repair
                )
            case ReferenceType.quote:
    ...
    def _validate_citation(
        self, citation: Reference, source_section: str, attempt_repair: bool
    ):
        """
        ...
        if attempt_repair:
            self._print(
                f"   Validating citation: {citation.reference_str} with reference {citation.text}"
            )
        system_message = """
...
"""
        ...
        if attempt_repair:
            self._print(
                f"   Invalid citation couldn't be justified to have come from\n   {source_section}"
            )
        else:
            return False
    ...
    def _validate_quote(
        self, quote: Reference, source_section: str, attempt_repair: bool
    ):
        """
        ...
        if attempt_repair:
            self._print(
                f"   Invalid quote {quote.to_string()} not found in\n{source_section}"
            )
        else:
            return False
    ...
    def _get_potential_locations(self, reference: Reference):
        ...
        for location_str in sections_to_search_around:
            ...
            if len(parts) == 1:
                ...
                preceding_sections = map(
                    lambda args: self._parse_section(*args),
                    [(section - i, paragraph, subparagraph) for i in range(1, 3)],
                )
                succeeding_sections = map(
                    lambda args: self._parse_section(*args),
                    [(section + i, paragraph, subparagraph) for i in range(1, 3)],
                )
                ...
                preceding_sections = map(
                    lambda args: self._parse_section(*args),
                    [(section, paragraph - i, subparagraph) for i in range(1, 3)],
                )
                ...
                preceding_sections = map(
                    lambda args: self._parse_section(*args),
                    [(section, paragraph, subparagraph - i) for i in range(1, 3)],
                )
            ...
            possible_locations.append(
                self._parse_section(section + 1, paragraph, subparagraph)
            )
            possible_locations.append(
                self._parse_section(section - 1, paragraph, subparagraph)
            )
