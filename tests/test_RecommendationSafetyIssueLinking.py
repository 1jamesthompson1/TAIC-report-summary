from engine.analyze.RecommendationSafetyIssueLinking import RecommendationSafetyIssueLinker
...
class TestRecommendationSafetyIssueLinking:
    ...
    def test_single_report_linking(self):
        ...
        links = RecommendationSafetyIssueLinker()._evaluate_all_possible_links(test_dataframe['recommendations'][0], test_dataframe['safety_issues'][0])

        assert isinstance(links, pd.DataFrame)

    def test_multiple_reports_linking(self):

        RecommendationSafetyIssueLinker().evaluate_links_for_report('tests/data/extracted_reports.pkl', 'tests/data/links_temp.pkl')
