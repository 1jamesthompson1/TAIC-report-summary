...
def test_header_randomization():
    ...
    with tempfile.NamedTemporaryFile(suffix='.pkl', delete=False) as tmp_file:
        ...
        try:
            ...
            unique_accept_languages = set(h.get('Accept-Language') for h in headers_sets)
            unique_referers = set(h.get('Referer') for h in headers_sets)
            
            # Verify we have some variation
            assert len(unique_user_agents) > 1, "Should have multiple User-Agent variations"
            assert len(unique_accept_languages) > 1, "Should have multiple Accept-Language variations"
            ...
            for i, headers in enumerate(headers_sets):
                ...
                assert 'Accept-Language' in headers, f"Headers set {i} missing Accept-Language"
                ...
                assert 'Mozilla' in headers['User-Agent'], f"Headers set {i} has unrealistic User-Agent"
                
                # Verify headers are well-formed
                assert headers['Accept'].startswith('text/html'), f"Headers set {i} has malformed Accept header"
                assert headers['Accept-Language'].count(',') >= 0, f"Headers set {i} has malformed Accept-Language header"
                assert headers['Referer'].startswith('https://'), f"Headers set {i} has malformed Referer header"
                assert headers['Connection'] in ['keep-alive', 'close'], f"Headers set {i} has invalid Connection header"
